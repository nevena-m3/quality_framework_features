"""QADD v4.1 measurement functions for extrinsic additive acoustic interference.
...
MEASUREMENT_VERSION = "qadd-v4.1.0"
...
class QADDParameters:
    """Frozen signal-processing and minimum-support parameters for QADD v4.1.
...
def _raw_feature_estimates(
    ...
    return {
        ...
        "qadd_mains_hum_supported_harmonic_count_median": (
            float(
                pd.to_numeric(
                    winner_windows["hum_supported_harmonic_count"], errors="coerce"
                ).median()
            )
...
def extract_qadd(
    ...
    *,
    ...
    """Extract the complete QADD v4.1 vector and audit ledgers for one recording."""
    ...
    if int(sample_rate) != parameters.analysis_sample_rate_hz:
        raise ValueError(
            "QADD v4.1 requires the frozen 16-kHz analysis view; "
