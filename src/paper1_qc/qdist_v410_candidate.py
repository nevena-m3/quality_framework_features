"""QDIST v4.1.0 candidate: native-waveform hard-clipping morphology.
...
PRIMARY_FEATURES = ("qdist_hard_clipped_sample_fraction",)
SECONDARY_FEATURES = ("qdist_hard_clip_event_rate_per_min",)
CONDITIONAL_FEATURES = ("qdist_hard_clipped_frame_fraction",)
...
class QDISTParameters:
    """Prespecified engineering and minimum-support parameters for QDIST v4.1.0.
...
FEATURE_DEFINITIONS = (
    FeatureDefinition(
        ...
        "conditional analysis-window diagnostic",
        "fraction [0,1]",
        ...
        "Hard-clipping morphology only; the value depends on the arbitrary 30-ms "
        "frame-grid origin and is not a direct duration burden.",
        ...
        "Hansen et al. (2021); Laguna & Lerch (2016); Goldsack et al. (2020)",
    ),
    FeatureDefinition(
        "qdist_hard_clip_event_rate_per_min",
        "Hard-clipping episode rate",
        "clipping episode frequency",
        "secondary event metric",
        "events/min",
        "literature-informed engineering detector; study-specific aggregation",
        ...
        "Episode grouping depends on the frozen merge rule; events are observable "
        "plateau clusters, not directly observed device-state episodes.",
        "At least 3 s finite task-span exposure; event count and exposure are retained.",
        "Merge-gap choice, fragmented codec-smeared plateaus, quantization, speech "
        "amplitude/phonetic content, and sparse-count uncertainty.",
        "merged episode ledger",
        "Hansen et al. (2021); Xia et al. (2018); Eaton & Naylor (2013, 2014)",
    ),
    FeatureDefinition(
        "qdist_hard_clipped_sample_fraction",
        "Hard-clipped channel-sample fraction",
        "clipped waveform burden",
        "primary direct burden",
        "fraction [0,1]",
        "literature-informed engineering detector; study-specific aggregation",
        ...
        "At least 3 s task-span exposure with all channel-samples finite.",
        "Channel count, quantization, codec smearing, and event-boundary tolerance.",
        "accepted plateau ledger",
        "Hansen et al. (2021); Xia et al. (2018); Maymon et al. (2013)",
    ),
)
...
def _candidate_rows_for_channel(
    ...
    *,
    ...
    # cohort and admitted implausible near-zero plateaus. QDIST v4.1.0 requires
    ...
    for pair_start, pair_end in _true_runs(pair_flat):
        ...
        pre_same_polarity = polarity * pre_finite
        pre_same_polarity = pre_same_polarity[pre_same_polarity > 0]
        post_same_polarity = polarity * post_finite
        post_same_polarity = post_same_polarity[post_same_polarity > 0]
        pre_same_polarity_peak = (
            float(np.max(pre_same_polarity)) if pre_same_polarity.size else np.nan
        )
        post_same_polarity_peak = (
            float(np.max(post_same_polarity)) if post_same_polarity.size else np.nan
        )
        same_polarity_context_peak = (
            float(np.nanmax([pre_same_polarity_peak, post_same_polarity_peak]))
            if np.isfinite(pre_same_polarity_peak)
            or np.isfinite(post_same_polarity_peak)
            else np.nan
        )
        entry = (
        ...
        local_magnitude_ratio = (
            abs_level / same_polarity_context_peak
            if np.isfinite(same_polarity_context_peak)
            and same_polarity_context_peak > 0
            else np.nan
        )
        ...
        rows.append(
            {
                ...
                "local_same_polarity_context_peak_abs": same_polarity_context_peak,
