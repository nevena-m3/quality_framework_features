"""QCHAN v4.0.0: cohort-relative channel/device spectral descriptors.
...
def analysis_waveform_from_audio_views(views: object) -> np.ndarray:
    ...
    QCHAN is an analysis-view family.  The helper intentionally rejects
    historical aliases so the reviewed notebook cannot silently use native
    audio or a noncanonical attribute.
    """

    if not hasattr(views, "analysis_16k"):
        raise AttributeError(
            "QCHAN requires AudioViews.analysis_16k; the canonical analysis "
            "waveform field is missing."
        )
    waveform = np.asarray(getattr(views, "analysis_16k"), dtype=np.float64)
    if waveform.ndim != 1:
        raise ValueError("AudioViews.analysis_16k must be one-dimensional.")
    if waveform.size == 0:
...
def reference_from_recording_spectrum(
    spectrum: RecordingSpectrum,
    *,
    reference_key: str = "controlled_baseline",
    task_stratum: str = "controlled_fixture",
    excluded_subject_id: str = "target",
    parameters: QChanParameters = DEFAULT_PARAMETERS,
) -> ReferenceSpectrum:
    """Create a deterministic single-spectrum controlled reference.

    This helper is only for analytical fixtures.  Cohort extraction must use
    :func:`build_subject_balanced_loso_references`.
    """

    if spectrum.status != "measured":
        raise ValueError("Controlled reference spectrum must be measured.")
    payload = {
        "reference_key": reference_key,
    ...
    reference_hash = _stable_hash(payload)
    vintage = _stable_hash({
        "measurement_version": MEASUREMENT_VERSION,
        "controlled_reference": payload,
    })
    return ReferenceSpectrum(
        reference_key=reference_key,
        task_stratum=task_stratum,
        excluded_subject_id=excluded_subject_id,
        ...
        status="measured",
        member_recording_ids=(spectrum.logical_recording_id,),
        member_subject_ids=("controlled_reference",),
        recording_count=1,
        subject_count=1,
        reference_sha256=reference_hash,
        reference_vintage_sha256=vintage,
...
def extract_controlled_features(
    waveform: np.ndarray,
    *,
    reference: ReferenceSpectrum,
    logical_recording_id: str,
    ...
    """Extract reviewed QCHAN features against a controlled reference."""

    observation = extract_recording_spectrum(
        waveform,
        parameters.analysis_sample_rate_hz,
        strict_speech=full_span_interval(
            waveform, parameters.analysis_sample_rate_hz
        ),
        logical_recording_id=logical_recording_id,
        source_sample_rate_hz=source_sample_rate_hz,
        parameters=parameters,
    )
    return compute_reference_relative_features(
        observation, reference, parameters
    )
