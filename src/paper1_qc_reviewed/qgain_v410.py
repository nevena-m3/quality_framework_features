"""QGAIN v4.1.0 reviewed final: recorded level and level dynamics.
...
CANONICAL_SEGMENTATION_PROFILE = "primary"
...
# Backward-compatible alias used only for availability summaries.
PRIMARY_FEATURES = PRIMARY_ANALYSIS_FEATURES
...
FEATURE_DEFINITIONS = (
    ...
    FeatureDefinition(
        ...
        publication_role="secondary mixed acquisition/physiology descriptor",
        default_manuscript_inclusion=False,
        exploratory=False,
        robustness_class="moderately_sensitive",
        interpretation_class="mixed acquisition/physiology descriptor",
        unit="dB",
        ...
        phenotype_confounding_risk="high",
        ml_role="secondary biomarker-reliability covariate; low-support cases require mask/tier",
        standalone_gate_allowed=False,
        composite_use_prohibited=True,
        missing_value_behavior="NaN with explicit status/support; never imputed by extractor",
    ),
    FeatureDefinition(
        ...
        measurement_role="exploratory_contextual",
        ...
        ml_role="exploratory reliability/context covariate only; not acquisition-specific",
        standalone_gate_allowed=False,
        composite_use_prohibited=True,
        missing_value_behavior="NaN with explicit status/support; never imputed by extractor",
    ),
...
def select_canonical_speech_intervals(
    intervals_frame: pd.DataFrame,
    recording_id: str,
    *,
    view: str = CANONICAL_SPEECH_VIEW,
    profile: str = CANONICAL_SEGMENTATION_PROFILE,
) -> list[TimeInterval]:
    """Select exactly one frozen segmentation view/profile.

    The segmentation freeze contains multiple views and multiple parameter
    profiles. They must never be pooled or unioned for production extraction.
    QGAIN uses only ``strict_speech`` from the ``primary`` profile.
    """

    required = {"logical_recording_id", "view", "profile", "start_sec", "end_sec"}
    missing = sorted(required.difference(intervals_frame.columns))
    if missing:
        raise ValueError(f"Frozen interval table lacks required columns: {missing}")

    local = intervals_frame.loc[
        intervals_frame["logical_recording_id"].astype(str).eq(str(recording_id))
        & intervals_frame["view"].astype(str).eq(str(view))
        & intervals_frame["profile"].astype(str).eq(str(profile))
    ].copy()

    if "segmentation_analysis_eligible" in local:
        local = local.loc[local["segmentation_analysis_eligible"].map(_coerce_bool_value)]
    if "decision" in local:
        local = local.loc[local["decision"].astype(str).str.upper().eq("KEEP")]
    if local.empty:
        raise ValueError(
            f"No canonical {view}/{profile} intervals for recording {recording_id}."
        )

    sort_columns = [column for column in ["interval_index", "start_sec", "end_sec"] if column in local]
    local = local.sort_values(sort_columns, kind="mergesort")
    if "interval_index" in local and local["interval_index"].duplicated().any():
        raise ValueError(f"Duplicate canonical interval_index values for {recording_id}.")
    if local.duplicated(["start_sec", "end_sec"]).any():
        raise ValueError(f"Duplicate canonical interval boundaries for {recording_id}.")

    starts = pd.to_numeric(local["start_sec"], errors="coerce").to_numpy(float)
    ends = pd.to_numeric(local["end_sec"], errors="coerce").to_numpy(float)
    if not np.isfinite(starts).all() or not np.isfinite(ends).all() or np.any(ends <= starts):
        raise ValueError(f"Invalid canonical interval boundaries for {recording_id}.")
    order = np.argsort(starts, kind="mergesort")
    starts, ends = starts[order], ends[order]
    if len(starts) > 1 and np.any(starts[1:] < ends[:-1]):
        raise ValueError(f"Overlapping canonical intervals for {recording_id}.")
    return [TimeInterval(float(start), float(end)) for start, end in zip(starts, ends)]


def finalize_recording_frame_v410(frame: pd.DataFrame) -> pd.DataFrame:
    """Upgrade a validated v4.0.1 cohort table to the v4.1.0 semantic contract.

    Numerical feature values are not recomputed or altered. The function verifies
    the canonical segmentation contract, adds final role/evidence metadata, and
    updates availability summaries for the accepted feature hierarchy.
    """

    required = {
        "logical_recording_id",
        "qgain_speech_view",
        "qgain_segmentation_profile",
        *ANALYSIS_FEATURES,
    }
    missing = sorted(required.difference(frame.columns))
    if missing:
        raise ValueError(f"Validated cohort table lacks required columns: {missing}")
    upgraded = frame.copy()
    if not upgraded["qgain_speech_view"].astype(str).eq(CANONICAL_SPEECH_VIEW).all():
        raise ValueError("v4.1.0 finalization requires strict_speech for every recording.")
    if not upgraded["qgain_segmentation_profile"].astype(str).eq(CANONICAL_SEGMENTATION_PROFILE).all():
        raise ValueError("v4.1.0 finalization requires the primary segmentation profile.")
    if upgraded["logical_recording_id"].astype(str).duplicated().any():
        raise ValueError("Recording identities are not unique.")

    upgraded["qgain_measurement_version"] = MEASUREMENT_VERSION
    upgraded["qgain_ml_use_status"] = (
        "retained_non_gating_context_and_biomarker_reliability_covariates"
    )
    upgraded["qgain_decision_threshold_status"] = "not_calibrated"
    upgraded["qgain_standalone_reject_allowed"] = False
    upgraded["qgain_family_scalar_available"] = False
    upgraded["qgain_family_scalar_status"] = "prohibited_not_constructed"

    low = pd.to_numeric(
        upgraded.get("qgain_signed_drift_ci95_low_db_per_min"), errors="coerce"
    )
    high = pd.to_numeric(
        upgraded.get("qgain_signed_drift_ci95_high_db_per_min"), errors="coerce"
    )
    available = low.notna() & high.notna()
    excludes = available & ((low > 0) | (high < 0))
    evidence = np.where(
        ~available,
        "unavailable",
        np.where(excludes, "ci_excludes_zero", "ci_includes_zero"),
    )
    upgraded["qgain_drift_ci_excludes_zero"] = pd.Series(excludes, index=upgraded.index).astype(bool)
    upgraded["qgain_drift_evidence_status"] = evidence
    upgraded["qgain_drift_publication_role"] = "exploratory_contextual"

    primary_count = sum(
        pd.to_numeric(upgraded[name], errors="coerce").notna().astype(int)
        for name in PRIMARY_ANALYSIS_FEATURES
    )
    retained_count = sum(
        pd.to_numeric(upgraded[name], errors="coerce").notna().astype(int)
        for name in ANALYSIS_FEATURES
    )
    upgraded["qgain_primary_available_count"] = primary_count.astype(int)
    upgraded["qgain_primary_analysis_available_count"] = primary_count.astype(int)
    upgraded["qgain_primary_analysis_eligible"] = primary_count.eq(len(PRIMARY_ANALYSIS_FEATURES))
    upgraded["qgain_retained_available_count"] = retained_count.astype(int)
    upgraded["qgain_all_retained_available"] = retained_count.eq(len(ANALYSIS_FEATURES))
    upgraded["qgain_family_measurement_status"] = np.select(
        [
            upgraded["qgain_all_retained_available"],
            retained_count.gt(0),
            upgraded.get("qgain_floor_contaminated", False),
        ],
        ["all_retained_available", "partial_retained_available", "floor_contaminated"],
        default="unavailable",
    )
    return upgraded
...
def extract_qgain(
    ...
    *,
    ...
    speech_view: str = CANONICAL_SPEECH_VIEW,
    segmentation_profile: str = CANONICAL_SEGMENTATION_PROFILE,
) -> QGAINExtraction:
    ...
    recording = {
        ...
        "qgain_speech_view": str(speech_view),
        "qgain_segmentation_profile": str(segmentation_profile),
        ...
        "qgain_ml_use_status": "retained_non_gating_context_and_biomarker_reliability_covariates",
        "qgain_family_scalar_available": False,
        "qgain_family_scalar_status": "prohibited_not_constructed",
        **values,
    }
    ...
    primary_available = sum(np.isfinite(values[name]) for name in PRIMARY_ANALYSIS_FEATURES)
    retained_available = sum(np.isfinite(values[name]) for name in ANALYSIS_FEATURES)
    recording["qgain_primary_available_count"] = int(primary_available)
    recording["qgain_primary_analysis_available_count"] = int(primary_available)
    recording["qgain_primary_analysis_eligible"] = bool(
        primary_available == len(PRIMARY_ANALYSIS_FEATURES)
    )
    recording["qgain_retained_available_count"] = int(retained_available)
    recording["qgain_all_retained_available"] = bool(
        retained_available == len(ANALYSIS_FEATURES)
    )
    if retained_available == len(ANALYSIS_FEATURES):
        family_status = "all_retained_available"
    elif retained_available:
        family_status = "partial_retained_available"
    ...
    ci_available = np.isfinite(drift_ci_low) and np.isfinite(drift_ci_high)
    ci_excludes_zero = bool(ci_available and (drift_ci_low > 0 or drift_ci_high < 0))
    recording["qgain_drift_ci_excludes_zero"] = ci_excludes_zero
    recording["qgain_drift_evidence_status"] = (
        "ci_excludes_zero" if ci_excludes_zero
        else "ci_includes_zero" if ci_available
        else "unavailable"
    )
    recording["qgain_drift_publication_role"] = "exploratory_contextual"
            ...
            output.append(
                {
                    ...
                    "measurement_role": meta["measurement_role"],
                    "analysis_priority": meta["analysis_priority"],
                    "publication_role": meta["publication_role"],
                    "default_manuscript_inclusion": bool(meta["default_manuscript_inclusion"]),
                    "exploratory": bool(meta["exploratory"]),
                    "robustness_class": meta["robustness_class"],
...
def model_interface_frame(rows: Mapping[str, object] | pd.DataFrame) -> pd.DataFrame:
    ...
    wide = pd.DataFrame([rows]) if isinstance(rows, Mapping) else rows.copy()
    columns = ["logical_recording_id", "qgain_measurement_version", "qgain_signal_view", "qgain_speech_view", "qgain_segmentation_profile"]
    for feature in ANALYSIS_FEATURES:
        columns.extend(
            [
                feature,
    ...
    columns.extend(
        [
            ...
            "qgain_primary_analysis_available_count",
            "qgain_retained_available_count",
            "qgain_all_retained_available",
            ...
            "qgain_drift_publication_role",
            "qgain_family_scalar_available",
