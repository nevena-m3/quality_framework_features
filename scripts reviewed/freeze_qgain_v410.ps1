...
$NotebookDir = Join-Path $ProjectRoot "notebooks reviewed\01_QGAIN"
$Module = Join-Path $ProjectRoot "src reviewed\paper1_qc_reviewed\qgain_v410.py"
$Tests = Join-Path $ProjectRoot "tests reviewed\test_qgain_v410.py"
...
try {
    & $Python -m pytest "tests reviewed\test_qgain_v410.py" -q
    if ($LASTEXITCODE -ne 0) {
